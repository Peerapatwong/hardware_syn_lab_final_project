// ============================================================
// camera_config.v — SCCB master + config sequencer + register LUT
// Adapted from a proven working OV7670 Basys 3 project.
//
// Contains 3 modules:
//   I2C_Controller          — bit-bangs the SCCB write protocol
//   I2C_AV_Config           — sequences through the register LUT
//   I2C_OV7670_RGB565_Config — the 165-register init table
//
// The config sequencer RETRIES on NACK, which is critical —
// some cameras take extra time to respond.
// ============================================================

// ---- SCCB bit-bang controller ----
...
always @(posedge iCLK or negedge iRST_N) begin
    if (!iRST_N) begin
        ...
        END <= 0; I2C_RDATA <= 8'h0;
    end else if (I2C_EN) begin
        if (GO) begin
            if (WR) begin
                case (SD_COUNTER)
                    6'd0 : begin SCLK<=1; I2C_BIT<=1;
                           ACKW1<=1; ACKW2<=1; ACKW3<=1; END<=0; end
                    6'd1 : begin SCLK<=1; I2C_BIT<=1;
                           ACKW1<=1; ACKW2<=1; ACKW3<=1; END<=0; end
                    6'd2 : I2C_BIT <= 0;
                    6'd3 : SCLK <= 0;
...
// ---- Config sequencer: walks through register LUT, retries on NACK ----
...
parameter LUT_SIZE = 167;   // 2 read + 165 write entries

// 10 kHz I2C clock from 25 MHz
            ...
            case (mSetup_ST)
                0: begin
                    if (~mI2C_END)
                        ...
                        mSetup_ST <= 0;
                    mI2C_GO <= 1;
                    mI2C_WR <= (LUT_INDEX >= 8'd2) ? 1'b1 : 1'b0;
                end
                1: begin
                    if (mI2C_END) begin
                        mI2C_WR <= 0;
                        mI2C_GO <= 0;
                        if (~mI2C_ACK)
                            mSetup_ST <= 2;  // ACK OK → next register
                        else
                            mSetup_ST <= 0;  // NACK → retry same register
                    end
                end
                2: begin
...
I2C_OV7670_RGB565_Config OV7670_LUT (
    .LUT_INDEX (LUT_INDEX),
    .LUT_DATA  (LUT_DATA)
);

I2C_Controller sccb_ctrl (
    .iCLK      (iCLK),
    .iRST_N    (iRST_N),
...
// ---- 165-register init table (PROVEN WORKING for QVGA RGB565) ----
...
always @(*) begin
    case (LUT_INDEX)
    // --- Format: QVGA + RGB565 ---
    SET_OV7670+0  : LUT_DATA = 16'h1214;  // COM7   – QVGA + RGB
    SET_OV7670+1  : LUT_DATA = 16'h40d0;  // COM15  – RGB565, full range
    SET_OV7670+2  : LUT_DATA = 16'h3a04;  // TSLB
    SET_OV7670+3  : LUT_DATA = 16'h3dc8;  // COM13
    SET_OV7670+4  : LUT_DATA = 16'h1e31;  // MVFP – mirror+flip+blacksun
    SET_OV7670+5  : LUT_DATA = 16'h6b00;  // DBLV – bypass PLL
    SET_OV7670+6  : LUT_DATA = 16'h32b6;  // HREF
    SET_OV7670+7  : LUT_DATA = 16'h1713;  // HSTART
    SET_OV7670+8  : LUT_DATA = 16'h1801;  // HSTOP
    SET_OV7670+9  : LUT_DATA = 16'h1902;  // VSTART
    SET_OV7670+10 : LUT_DATA = 16'h1a7a;  // VSTOP
    SET_OV7670+11 : LUT_DATA = 16'h030a;  // VREF
    SET_OV7670+12 : LUT_DATA = 16'h0c00;  // COM3
    SET_OV7670+13 : LUT_DATA = 16'h3e00;  // COM14
    SET_OV7670+14 : LUT_DATA = 16'h7000;  // SCALING_XSC
    SET_OV7670+15 : LUT_DATA = 16'h7100;  // SCALING_YSC
    SET_OV7670+16 : LUT_DATA = 16'h7211;  // SCALING_DCWCTR
    SET_OV7670+17 : LUT_DATA = 16'h7300;  // SCALING_PCLK_DIV
    SET_OV7670+18 : LUT_DATA = 16'ha202;  // SCALING_PCLK_DELAY
    SET_OV7670+19 : LUT_DATA = 16'h1180;  // CLKRC – ext clock direct
    // --- Gamma curve ---
    ...
    // --- AGC/AEC/AWB ---
    SET_OV7670+36 : LUT_DATA = 16'h13e0;  // COM8 – disable AGC/AWB/AEC
    SET_OV7670+37 : LUT_DATA = 16'h0000;  // GAIN
    SET_OV7670+38 : LUT_DATA = 16'h1000;  // AECH
    SET_OV7670+39 : LUT_DATA = 16'h0d00;  // COM4
    SET_OV7670+40 : LUT_DATA = 16'h1428;  // COM9 – 8x AGC ceiling
    SET_OV7670+41 : LUT_DATA = 16'ha505;  // BD50MAX
    SET_OV7670+42 : LUT_DATA = 16'hab07;  // BD60MAX
    SET_OV7670+43 : LUT_DATA = 16'h2475;  // AEW
    SET_OV7670+44 : LUT_DATA = 16'h2563;  // AEB
    SET_OV7670+45 : LUT_DATA = 16'h26a5;  // VPT
    SET_OV7670+46 : LUT_DATA = 16'h9f78;  // HAECC1
    SET_OV7670+47 : LUT_DATA = 16'ha068;  // HAECC2
    SET_OV7670+48 : LUT_DATA = 16'ha103;  // HAECC3 (was RSVD)
    SET_OV7670+49 : LUT_DATA = 16'ha6df;  // HAECC3
    SET_OV7670+50 : LUT_DATA = 16'ha7df;  // HAECC4
    SET_OV7670+51 : LUT_DATA = 16'ha8f0;  // HAECC5
    SET_OV7670+52 : LUT_DATA = 16'ha990;  // HAECC6
    SET_OV7670+53 : LUT_DATA = 16'haa94;  // HAECC7
    SET_OV7670+54 : LUT_DATA = 16'h13ef;  // COM8 – re-enable AGC/AWB/AEC
    // --- Misc ---
    ...
    SET_OV7670+106: LUT_DATA = 16'h0140;  // BLUE gain
    SET_OV7670+107: LUT_DATA = 16'h0240;  // RED gain
    SET_OV7670+108: LUT_DATA = 16'h13e7;  // COM8 – final AGC/AWB/AEC
    SET_OV7670+109: LUT_DATA = 16'h1500;
    // --- Color matrix ---
    SET_OV7670+110: LUT_DATA = 16'h4f80;  // MTX1
    SET_OV7670+111: LUT_DATA = 16'h5080;  // MTX2
    SET_OV7670+112: LUT_DATA = 16'h5100;  // MTX3
    SET_OV7670+113: LUT_DATA = 16'h5222;  // MTX4
    SET_OV7670+114: LUT_DATA = 16'h535e;  // MTX5
    SET_OV7670+115: LUT_DATA = 16'h5480;  // MTX6
    SET_OV7670+116: LUT_DATA = 16'h589e;  // MTXS
    // --- Edge / denoise ---
